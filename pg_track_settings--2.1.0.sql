...
CREATE UNLOGGED TABLE @extschema@.pg_track_settings_settings_src_tmp (
...
CREATE TABLE @extschema@.pg_track_settings_list (
    srvid integer NOT NULL,
    name text,
    PRIMARY KEY (srvid, name)
);
SELECT pg_catalog.pg_extension_config_dump('@extschema@.pg_track_settings_list', '');

CREATE TABLE @extschema@.pg_track_settings_history (
    srvid integer NOT NULL,
...
SELECT pg_catalog.pg_extension_config_dump('@extschema@.pg_track_settings_history', '');

CREATE UNLOGGED TABLE @extschema@.pg_track_settings_rds_src_tmp (
    srvid integer NOT NULL,
    ts timestamp with time zone NOT NULL,
    name text NOT NULL,
    setting text,
...
CREATE TABLE @extschema@.pg_track_db_role_settings_list (
    srvid integer,
    name text,
    setdatabase oid,
    setrole oid,
...
SELECT pg_catalog.pg_extension_config_dump('@extschema@.pg_track_db_role_settings_list', '');

CREATE TABLE @extschema@.pg_track_db_role_settings_history (
    srvid INTEGER NOT NULL,
    ts timestamp with time zone,
    name text,
    setdatabase oid,
...
SELECT pg_catalog.pg_extension_config_dump('@extschema@.pg_track_db_role_settings_history', '');

CREATE UNLOGGED TABLE @extschema@.pg_track_settings_reboot_src_tmp (
    srvid integer NOT NULL,
    ts timestamp with time zone NOT NULL,
...
CREATE TABLE @extschema@.pg_reboot (
    srvid integer NOT NULL,
    ts timestamp with time zone,
    PRIMARY KEY (srvid, ts)
);
SELECT pg_catalog.pg_extension_config_dump('@extschema@.pg_reboot', '');
...
CREATE OR REPLACE FUNCTION @extschema@.pg_track_settings_snapshot_reboot(_srvid integer)
    RETURNS boolean AS
$_$
BEGIN
    ...
    WITH last_reboot AS (
        ...
        FROM @extschema@.pg_track_settings_reboot_src(_srvid) t
    )
    INSERT INTO @extschema@.pg_reboot (srvid, ts)
    SELECT _srvid, lr.postmaster_ts FROM last_reboot lr
    WHERE NOT EXISTS (SELECT 1
        FROM @extschema@.pg_reboot r
    ...
    IF (_srvid != 0) THEN
        DELETE FROM @extschema@.pg_track_settings_reboot_src_tmp
        WHERE srvid = _srvid;
    END IF;
...
$_$
BEGIN
    PERFORM @extschema@.pg_track_settings_snapshot_settings(0);
    PERFORM @extschema@.pg_track_settings_snapshot_rds(0);
    PERFORM @extschema@.pg_track_settings_snapshot_reboot(0);
...
CREATE OR REPLACE FUNCTION @extschema@.pg_track_settings_diff(
...
BEGIN
    RETURN QUERY
        ...
        FROM @extschema@.pg_track_settings(_from, _srvid) s1
        FULL OUTER JOIN @extschema@.pg_track_settings(_to, _srvid) s2 ON s2.name = s1.name
...
CREATE OR REPLACE FUNCTION @extschema@.pg_track_db_role_settings_diff(
    _from timestamp with time zone,
    _to timestamp with time zone,
    _srvid integer DEFAULT 0)
...
BEGIN
    RETURN QUERY
        SELECT COALESCE(s1.setdatabase, s2.setdatabase),
               ...
               s2.setting AS to_setting,
               CASE WHEN s2.setting IS NULL THEN false ELSE true END
        FROM @extschema@.pg_track_db_role_settings(_from, _srvid) s1
        FULL OUTER JOIN @extschema@.pg_track_db_role_settings(_to, _srvid) s2 ON
...
CREATE OR REPLACE FUNCTION @extschema@.pg_track_settings_reset(_srvid integer DEFAULT 0)
RETURNS void AS
$_$
BEGIN
    DELETE FROM @extschema@.pg_track_settings_settings_src_tmp WHERE srvid = _srvid;
    DELETE FROM @extschema@.pg_track_settings_rds_src_tmp WHERE srvid = _srvid;
    DELETE FROM @extschema@.pg_track_settings_reboot_src_tmp WHERE srvid = _srvid;
    DELETE FROM @extschema@.pg_track_settings_list WHERE srvid = _srvid;
    DELETE FROM @extschema@.pg_track_settings_history WHERE srvid = _srvid;
    DELETE FROM @extschema@.pg_track_db_role_settings_list WHERE srvid = _srvid;
    DELETE FROM @extschema@.pg_track_db_role_settings_history WHERE srvid = _srvid;
    DELETE FROM @extschema@.pg_reboot WHERE srvid = _srvid;
END;
$_$
LANGUAGE plpgsql;
