...
--
-- Copyright (C) 2015-2019: Julien Rouhaud
...
BEGIN
    ...
    last_snapshot AS (
        ...
        FROM (
            SELECT setdatabase, setrole, name, setting,
                row_number() OVER (PARTITION BY NAME ORDER BY ts DESC) rownum
...
RETURNS TABLE (setdatabase oid, setrole oid, name text, from_setting text, from_exists boolean, to_setting text, to_exists boolean) AS
$_$
BEGIN
    RETURN QUERY
...
RETURNS TABLE (ts timestamp with time zone, setdatabase oid, setrole oid, name text, setting_exists boolean, setting text) AS
$_$
BEGIN
    RETURN QUERY
